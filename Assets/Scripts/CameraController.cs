...
public class CameraController : MonoBehaviour {
	...
	void Update () {
		Vector3 translation = GetComponent<Transform>().InverseTransformVector(
            new Vector3(
		...
		if(Input.GetButton("Fire2") && !Input.GetButtonDown ("Fire2"))
			translation += GetComponent<Transform>().InverseTransformVector(
                new Vector3(-Input.GetAxis("Mouse X"), -Input.GetAxis("Mouse Y"), 0) * mSpeed);
